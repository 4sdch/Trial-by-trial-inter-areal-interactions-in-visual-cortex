...
def plot_correlations(df_, color, r_position, ax, ev=None):
    ...
    if ev is None:
        ev = df_['actual'].corr(df_['predicted'])**2
    s = f"EV={ev:.2f}"

    ax.text(x=r_position[0], y=r_position[1], s=s, 
            fontsize=6, transform=ax.transAxes)
    ax.spines[:].set_linewidth(0.3)
    sns.despine()

def plot_three_neurons(frame_start, frame_stop, resp, predictions, reordered_neurons, 
                    color, axes, animal='mouse', ylim=None, condition_type=None, frames_per_trial=16):
    ...
    if animal=='mouse':
        ...
        frame_indices = np.arange(frames_per_trial*frame_start, frames_per_trial*frame_stop)
    ...
    for neuron, ax in enumerate(axes.flat):
        ...
        if animal=='monkey':
            if condition_type =='SNR':
                ax.set_xticks(ticks=np.arange(frames_per_trial*frame_start, frames_per_trial*frame_stop, frames_per_trial*2), labels=np.arange(frame_start, frame_stop, 2))
    ...
    for dataset_type in dataset_types:
        ...
        for mouse, areas_data in mouse_stats_[dataset_type].items():
            ...
            for area, values in areas_data.items():
                ...
                for cell_n, (split_half_r, snr, evar, null_evar) in enumerate(zip(split_half_rs, SNRs,values['evars'],values['evars_null'])):
...
def get_ext_fig_stars(df_, x_label='Mouse Name', dep_var ='EV', 
    ...
    for label in df_[x_label].unique():
        pvalnames, stars  = get_group_pairwise_stars(df_[df_[x_label]==label], dependent_variable=ind_var, dependent_variable_order=df_[ind_var].unique(),
...
def make_monkey_df_poisson(monkey_stats_, dataset_types=['SNR', 'RF_thin', 'RF_large'], add_glm_ev = False, prediction_type_comparison='poisson_glm'):
    """
    Create a DataFrame from the provided monkey statistics data for different dataset types.
    ...
    for dataset_type in dataset_types:
        ...
        for date, areas_data in monkey_stats_[dataset_type].items():
            ...
            for area, values in areas_data.items():
                ...
                SNRs = monkey_stats_[get_property_dataset_type_monkey(dataset_type)][date][area]['SNR_meanspont']
                if add_glm_ev:
                    print(f'monkey date: {date}, area: {area}, prediction type comparison: {prediction_type_comparison}')
                    if 'poisson_glm' in prediction_type_comparison:
                        pred_type_comparison_ = 'glm'
                    else:
                        pred_type_comparison_ = prediction_type_comparison
                    EV_glm = values[f'evars_{pred_type_comparison_}']
                    EV_glm_null = values[f'evars_null_{pred_type_comparison_}']
                else:
                    EV_glm = [np.nan]*len(values['evars'])
                    EV_glm_null = [np.nan]*len(values['evars'])
                for cell_n, (split_half_r, snr, evar, null_evar) in enumerate(zip(split_half_rs, SNRs,values['evars'],values['evars_null'])):
                    # Append data for the actual experiment (control_shuffle = False)
                    data.append({
                        'Dataset Type': dataset_type,
