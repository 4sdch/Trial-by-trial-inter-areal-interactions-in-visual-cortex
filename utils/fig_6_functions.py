...
from stats_functions import get_comparison_test_stars, get_group_pairwise_stars
...
from neuron_properties_functions import get_dates
from statsmodels.stats.multitest import multipletests
from utils.stats_functions import perm_test, perm_test_paired, hierarchical_permutation_test
...
def get_one_vs_rest_r_monkey_all_dates(monkey_stats, w_size=25, monkey='L', dataset_types=['RF_large','SNR','RF_thin']):
    """
    ...
    for dataset_type in dataset_types:
        ...
        for date in dates:
            if 'RF' in dataset_type:
                ...
                binned_epochs = get_img_resp_avg_sem(resp_V1, condition_type=dataset_type, get_chunks=True) 
...
def get_RF_filtered_df(monkey='L', date='250717', condition_type='SNR'):
    ...
    if 'RF' in condition_type:
        date = '250717'
    SNR_df = pd.read_csv(f'data/chen/metadata/monkey_{monkey}/{monkey}_SNR_{date}_full.csv')
    area_mapping = pd.read_csv(f'data/chen/metadata/monkey_{monkey}/channel_area_mapping_{monkey}.csv')
    SP = pd.read_csv(f'data/chen/metadata/monkey_{monkey}/{monkey}_RS_{date}_removal_metadata.csv')
    ...
    RF_df = pd.read_csv(f'data/chen/metadata/monkey_{monkey}/combined_{monkey}_RF.csv')
...
def get_electrode_ids_all_dates(monkey_stats_, monkey='L'):
    """
    ...
    for condition_type in monkey_stats_:
        ...
        for date in monkey_stats_[condition_type]:
            if date in ['140819', '150819', '160819']:
                continue
            df = get_RF_filtered_df(monkey=monkey, date=date, condition_type=condition_type)
...
def get_predictor_indices_elec_ids(monkey_stats, condition_type, get_condition_type, target_x_n=14, percent_over=80, percent_under=10, monkey='L'):
    """
    ...
    for date in dates:
        ...
        if date in ['140819', '150819', '160819']:
            continue
        RF_df= get_RF_filtered_df(monkey=monkey, date=date, condition_type=condition_type)
        ...
        monkey_stats[condition_type][date][area][f'rf_overlap_over{percent_over}_under{percent_under}_{target_x_n}_predictors_chosen_indices_elec_ids']=(chosen_v4_indices,chosen_v4_elec_ids)
        monkey_stats[condition_type][date][area2][f'rf_overlap_over{percent_over}_under{percent_under}_{target_x_n}_predictors_chosen_indices_elec_ids']=(chosen_v1_indices,chosen_v1_elec_ids)
        ...
        print(f'Finished {condition_type} {date} {area} {area2} RF overlap predictors')
def get_predictors_elecs_seeds(RF_df, v_idx, v_elec, predictor_dict, seeds, target_x_n, percent_over, percent_under):
    ...
    for s, seed in enumerate(seeds):
        if len(high_elecs) < target_x_n or len(low_elecs) < target_x_n:
            raise ValueError(f"Not enough predictors: need at least {target_x_n}, but got "
                            f"{len(high_elecs)} high and {len(low_elecs)} low for v_idx={v_idx}, v_elec={v_elec}.")
        np.random.seed(seed)
...
def get_xtarget_predictor_indices_elecs(monkey_stats, condition_type, get_condition_type, seeds, ref_area, target_x_n=14, percent_over=80, percent_under=10, monkey='L'):
    """
    ...
    for date in dates:
        if date in ['140819', '150819', '160819']:
            continue
        RF_df= get_RF_filtered_df(monkey=monkey, date=date, condition_type=condition_type)
        chosen_indices,chosen_elec_ids = monkey_stats[condition_type][date][ref_area][f'rf_overlap_over{percent_over}_under{percent_under}_{target_x_n}_predictors_chosen_indices_elec_ids']
        ...
        print(f'Finished {condition_type} {date} {area} {area2} R xtarget overlap predictors')
...
def store_macaque_overlap_alphas(monkey_stats,n_splits=5, w_size=25,
                        ...
                        date_used = '090817', target_x_n=14, monkey='L'):
...
def get_x_target_overlap_evars(monkey_stats, condition_type, get_condition_type, seeds, 
                                ...
                                monkey='L', date_used='090817', condition_type_used='RS'):
    """
    ...
    if f'monkey_overlap_alphas_{target_x_n}' not in list(monkey_stats.keys()):
        print('Calculating and storing macaque overlap alphas')
        store_macaque_overlap_alphas(monkey_stats,n_splits=5, w_size=25,
                        sample_size=500, condition_type_used=condition_type_used, 
                        date_used =date_used, target_x_n=target_x_n, monkey=monkey)
    else:
        print(f'Macaque overlap alphas already stored')
        print('target_x_n', target_x_n)
        print(monkey_stats[f'monkey_overlap_alphas_{target_x_n}'])
        
    ref_alpha = monkey_stats[f'monkey_overlap_alphas_{target_x_n}'][sample_size][ref_area]
    print(f'using alpha {ref_alpha} for {ref_area} reference area')
    dates= monkey_stats[condition_type].keys()
    ...
    for date in dates:
        if date in ['140819', '150819', '160819']:
            continue
        # Get response data
        resp_V4, resp_V1 =get_resps(condition_type=get_condition_type, date=date, w_size=w_size, stim_on=stim_on, stim_off=stim_off, monkey=monkey)
        ...
        print(f'Finished {condition_type} {date} R xtarget overlap evars')
...
def get_property_dataset_type(input_string):
    if 'spont' in input_string:
        ...
        return input_string 
def make_mouse_df_neuron_properties(mouse_stats_, dataset_types=['ori32','natimg32'], 
                                    remove_pcs=False, verbose=False):
    rem_pc = ''
    if remove_pcs is True:
        rem_pc='_removed_32_pcs'
    data = []
    for dataset_type in dataset_types:
        ...
        for mouse, areas_data in mouse_stats_[dataset_type].items():
            if verbose:
                print('making df for', dataset_type, mouse)
            ...
            for area, values in areas_data.items():
                ...
                if act_type == 'gray screen':
                    ...
                    rem_pc_ = rem_pc
                # print lengths of all to make sure they match
                # print('making df for', dataset_type, mouse, area_)
                # print(len(split_half_rs), len(SNRs), len(values['max_corr_val'+ rem_pc_]), len(variances), len(one_vs_rests), len(values['evars'+ rem_pc_]), len(values['evars_null']))
...
def make_monkey_df_neuron_properties(monkey_stats_, dataset_types=['SNR', 'RF_thin', 'RF_large'], verbose=False, get_variances=True):
    data = []
    for dataset_type in dataset_types:
        ...
        for date, areas_data in monkey_stats_[dataset_type].items():
            ...
            for area, values in areas_data.items():
                if verbose:
                    print('making df for', dataset_type, date, area)
                if area=='V4':
                    direction = 'V1→V4'
                else:
                    direction = 'V4→V1'
                ...
                if get_variances is True:
                    if 'RF' in dataset_type:
                        rf_variance_across_stimuli = monkey_stats_[get_property_dataset_type_monkey(dataset_type)][date][area]['var_across_stimuli']
                        var_within_trial_across_timepoints = np.mean(monkey_stats_[get_property_dataset_type_monkey(dataset_type)][date][area]['var_within_trial_across_timepoints'],axis=0)
                        var_within_timepoint_across_trials = np.mean(monkey_stats_[get_property_dataset_type_monkey(dataset_type)][date][area]['var_across_trials_within_timepoints'],axis=0)
                    else:
                        rf_variance_across_stimuli = [np.nan]*len(split_half_rs)
                        var_within_trial_across_timepoints = monkey_stats_[get_property_dataset_type_monkey(dataset_type)][date][area]['var_within_trial_across_timepoints']
                        var_within_timepoint_across_trials = monkey_stats_[get_property_dataset_type_monkey(dataset_type)][date][area]['var_across_trials_within_timepoints']
                else:
                    rf_variance_across_stimuli = [np.nan]*len(split_half_rs)
                    var_within_trial_across_timepoints = [np.nan]*len(split_half_rs)
                    var_within_timepoint_across_trials = [np.nan]*len(split_half_rs)
                for i, (split_half_r, snr,max_corr_val,onevsrest, evar, null_evar) in enumerate(zip(split_half_rs, SNRs,values['max_corr_val'],one_vs_rests,evars,evars_null)):
                    data.append({
                        'Dataset Type': dataset_type,
                        'Activity Type': act_type,
                        'Date':date,
                        ...
                        '1-vs-rest r²': np.square(onevsrest),
                        'control_shuffle':False, 
                        'Variance across stimuli': rf_variance_across_stimuli[i],
                        'Variance w/in trial\nacross timepoints': var_within_trial_across_timepoints[i],
                        'Variance w/in timepoint\nacross trials': var_within_timepoint_across_trials[i],
                    })
                    data.append({
                        'Dataset Type': dataset_type,
                        'Activity Type': act_type,
...
def add_stars_2_sets(df_, neuron_property, x, x_order, hue, ax, fontsize=7, height1=0.97, height2=0.97, perm_t=True, perm_type='ind', hierarchical=False, mouse_or_date='Mouse Name'):
    stars1 = get_comparison_test_stars(df_[df_[x]==x_order[0]], hue, neuron_property, perm_t=perm_t, perm_type=perm_type, hierarchical=hierarchical, mouse_or_date=mouse_or_date)
    stars2 = get_comparison_test_stars(df_[df_[x]==x_order[1]], hue, neuron_property, perm_t=perm_t, perm_type=perm_type,hierarchical=hierarchical, mouse_or_date=mouse_or_date)
    if stars1 == 'n.s.':
...
def make_corr_df_mouse(mouse_stats_, df_, 
                    value_types =['EV','max r² val', '1-vs-rest r²','SNR', 'Variance across stimuli',
       'Variance w/in stimulus\nacross trials','split-half r',] ):
    neuron_property='EV'
    data = []
...
def set_corr_figs_params(ax, fontsize=6, xticklabels=None):
    if xticklabels is None:
        xticklabels = ax.get_xticklabels()
    
    ax.set_xticks(ticks=ax.get_xticks(), labels=xticklabels, rotation=90)
    ax.set_xlabel(None)
...
def add_stars_5_sets(df_, neuron_property, x, x_order, hue, ax, fontsize=7, heights=None, x_positions=None):
    ...
    for s, xtick_label in enumerate(x_order):
        star =get_comparison_test_stars(df_[df_[x]==xtick_label], hue, neuron_property)
        if star =='n.s.':
...
def plot_mouse(df_, area, neuron_property, ax, y='EV', alpha=0.8, markersize=8,fontsize=6,r_pos = (0.05,0.95),
               r_val=None, downsample_fraction=None, unsorted=False, return_ax=False, **args):
    ...
    df_area = df_.query(area_query)
    if downsample_fraction is not None:
        df_area = df_area.sample(frac=downsample_fraction)
    if not unsorted:
        df_area_sorted = df_area.sort_values(by='EV gray screen')
    else:
        df_area_sorted = df_area
    sns.scatterplot(df_area_sorted, x=neuron_property, 
    y=y, color=color, ax=ax, alpha=alpha, s=markersize, **args)

    # Calculate the correlation
    if r_val is None:
        r_val = df_.query(area_query)[neuron_property].corr(df_.query(area_query)[y])
    sns.despine()

    ax.text(r_pos[0], r_pos[1], f'r={r_val:.2f}',fontsize=fontsize, transform=ax.transAxes)
...
def add_ax_properties_corr(a, ax, df_mouse_both, neuron_property, hide_xlabel=True, legend_title='EV gray screen', y_ticks=[0,0.5] ):
    ...
    if a!=2:
        ...
        legend.set_title(title=legend_title,prop={'size':6*0.8})
...
def make_corr_df_monkey(monkey_stats_, df_, value_types =['EV',
       'max r² val', '1-vs-rest r²', 'SNR', 'split-half r', 
       'Variance w/in trial\nacross timepoints',
       'Variance w/in timepoint\nacross trials','Variance across stimuli']):
    neuron_property='EV'
    data = []
    for dataset_type in df_['Dataset Type'].unique():
        if 'monkey' in dataset_type:
            continue
        for date in monkey_stats_[dataset_type]:
            ...
            for area in monkey_stats_[dataset_type][date]:
                ...
                # print('sub_df Dataset Type', dataset_type, 'area', area, 'date', date)
                for value_type in value_types: 
                    data.append({
                        'Dataset Type':dataset_type,
                        'Date': date,
...
def plot_bars(ax, heights=[1.1,1.2,1.3], x_position_left=-0.395, x_positions_right=[0.07,0.21,0.39], linewidth=0.4):
    
    for h, _ in enumerate(heights):
        ax.plot([x_position_left,x_positions_right[h]], # -0.095, units are width of the axes
            [heights[h], heights[h]], # 0 to 500, units are same as the y-axis
            clip_on=False,
...
def add_group_pairwise_stars_1_set_rf_overlap(df_, neuron_property,x,x1, hue, ax, 
    ...
    print('len of df_x1', len(df_x1))
    if hue_order is None:
        hue_order = df_x1[hue].unique()
    print('hue order', hue_order)
    pval_names ,all_stars1 = get_group_pairwise_stars(df_x1, hue,hue_order,neuron_property, perm_t=perm_t, perm_type=perm_type)
    height_positions = [height1, height2, height3]
    if stars1_positions is None:
        stars1_positions= [0.07,0.1,0.13]
    
    # orders all_stars1 according to hue_order pairwise comparisons
...
def add_group_pairwise_stars_1_set(df_, neuron_property, x, x1, hue, ax, fontsize=5, height1=0.73, 
                        height2=0.82,height3=0.91, height4=1, stars1_positions=None, stars2_positions=None,
                        perm_t=True, show_bar=True, bar_halflength = 0.035, n_comparisons=4):
    hue_order = df_[hue].unique()
    pval_names,all_stars1 = get_group_pairwise_stars(df_=df_[df_[x]==x1],dependent_variable=hue, dependent_variable_order=hue_order, neuron_property=neuron_property, perm_t=perm_t)
    height_positions1 = [height1,height2,height3,height4]
    if stars1_positions is None:
        stars1_positions= [0.14,0.19,0.23,0.27]
    
    offset_=0
    for s, (star1) in enumerate(all_stars1[:n_comparisons]):
        # print('count', s)
        if s>0:
            if show_bar is True:
...
def plot_corr_bars(df_monkey_corr, ax, area, remove_legend=True, hatch_size=3, 
                   fontsize=6, x='Value_Type',y='corr',hue='Activity Type', order=None, hue_order=None, **args):
    if order is None:
    ...
    if remove_legend is True:
        if area =='V4':
            palette=['#72BEB7','#B6E3DF','#136a66']
        elif area=='V1':
            palette=['#EDAEAE', '#f6d6d6','#a85959']
    else:
        palette = ['gray', 'lightgray','#3b3b3b'] 
    if hue_order is None: 
        hue_order= df_monkey_corr[hue].unique()
    sns.barplot(data=df_monkey_corr[mask], x=x, palette=palette,
    ...
    ax.set_xlabel(None)
    ax.set_ylabel('corr w/ EV fraction', fontsize=fontsize, labelpad=3, loc='bottom')
    ax.tick_params(axis='both', labelsize=fontsize, width=0.3, length=2, pad=1)
    if remove_legend is True:
        ax.legend_.remove()
    ax.spines[:].set_linewidth(0.5)
...
def plot_date(df_, area, neuron_property, ax, alpha=0.8, y='EV', hue='EV gray screen', 
              r_loc =(0.05,0.95), s=8,fontsize=6, r_val=None, **args):
    area_query=f'Area=="{area}"'
    ...
    sns.despine()
    ax.text(r_loc[0], r_loc[1], f'r={r_val:.2f}',fontsize=fontsize, transform=ax.transAxes)
    ax.tick_params(axis='both', labelsize=fontsize, width=0.5, pad=1)
...
def get_RF_filtered_df(monkey='L', date='250717', condition_type='SNR'):
    if 'RF' in condition_type and monkey=='L':
        date = '250717'
    elif 'RF' in condition_type and monkey=='A':
        date = '041018'
    SNR_df = pd.read_csv(f'data/chen/metadata/monkey_{monkey}/{monkey}_SNR_{date}_full.csv')
    area_mapping = pd.read_csv(f'data/chen/metadata/monkey_{monkey}/channel_area_mapping_{monkey}.csv')
    if monkey=='L':
        ...
        SP = pd.read_csv(f'data/chen/metadata/monkey_{monkey}/{monkey}_SNR_{date}_removal_metadata.csv')
    else: 
        raise ValueError('monkey must be L or A for now')
    overall_df = SNR_df.merge(area_mapping, on= ['Electrode_ID'])
    overall_df['SP_remove'] = overall_df['Electrode_ID'].isin(SP['Removed electrode ID'])
    overall_df['SNR_remove'] = overall_df['SNR']<2
    overall_df['SP_SNR_remove'] = (overall_df['Electrode_ID'].isin(SP['Removed electrode ID'])) | (overall_df['SNR']<2)
...
import copy
def create_df_overlaps(monkey_stats, condition_type, target_x_n=14, percent_over=60, percent_under=10, isolate_bar_sizes=False, monkey='L'):
    og_target_x_n= copy.deepcopy(target_x_n)
    # Flatten the nested dictionary into a DataFrame
    if 'spont' in condition_type:
        activity_type='gray screen'
    elif condition_type in ['RS','RS_open','RS_closed']:
        activity_type='lights off'
    else:
        activity_type='image'
    data = []
    for date, areas_data in monkey_stats[condition_type].items():
        ...
        if date in ['140819', '150819', '160819']:
            continue
        for area, values in areas_data.items():
            if area == 'V1' and monkey=='A':
                target_x_n=10
            else:
                target_x_n=og_target_x_n
            if area =="V4":
                area2='V1'
            elif area=="V1":
                area2='V4'
            if isolate_bar_sizes is True:
                ...
                if monkey=='A':
                    _,chosen_elec_ids = monkey_stats[condition_type][date][area][f'rf_overlap_over{percent_over}_under{percent_under}_{target_x_n}_predictors_chosen_indices_elec_ids']
                else:
                ...
                RF_info_df= get_RF_filtered_df(date=date, condition_type=condition_type, monkey=monkey)
                for n, idx in enumerate(chosen_indices):
                    all_predictor_over_elec_ids= values[f'predictor_target_indices_elecs_over80_percent_{target_x_n}_predictors_overlap_10_80_{target_x_n}'][1][n]
                    all_predictor_under_elec_ids= values[f'predictor_target_indices_elecs_under10_percent_{target_x_n}_predictors_overlap_10_80_{target_x_n}'][1][n]
...
def plot_ref_cell_overlap(condition_type, date, ref_elec, other_elecs, ax, color, markersize=3, point_size =2,
                        linewidth1=0.3, linewidth2=0.7, monkey='L'):
    RF_df_overall= get_RF_filtered_df(date=date, condition_type=condition_type, monkey=monkey)
...
def plot_cell_rf_overlaps(df_, refelec, condition_type, date, axes, fontsize=6, linewidth1=0.3, linewidth2=0.7,
                        markersize=3, point_size=3, monkey='L'):
    ...
    plot_ref_cell_overlap(condition_type, date,refelec, underpredictors, axes[0], 
                        ...
                        markersize=markersize, point_size=point_size, monkey=monkey)
...
def plot_null_line(df_, neuron_property, ax, color='blue', label='shuffle\ncontrol IQR'):
    ...
    ax.axhspan(per_25, per_75, alpha=0.4, color=color, label=label,
            linewidth=0)
...
def plot_spont_figs(df_both_comparisons_, area, neuron_property, x, hue, axes,threshold=None,low_threshold=None,
    ...
    for a, (ax, mouse_name) in  enumerate(zip(axes.flatten(), mouse_names)):
        ...
        ax.spines[:].set_linewidth(0.5)
        star =get_comparison_test_stars(mini_mouse_df, independent_variable=x, neuron_property=neuron_property, perm_type='paired')
        if star =='n.s.':
...
def plot_rf_overlap_performance(df_filtered_all, area, legend_loc=(1.01, 0.5), x='Activity_Type',
                                y='EV', order = None, **args):
    # Filter
    df_filtered = df_filtered_all[
        (df_filtered_all['SNR'] > 2) &
        (df_filtered_all['trad_reli'] > 0.8) &
        (df_filtered_all.control_shuffle == False)
    ]
    df_filtered_control = df_filtered_all[
        (df_filtered_all['SNR'] > 2) &
        (df_filtered_all['trad_reli'] > 0.8) &
        (df_filtered_all.control_shuffle == True)
    ]
    ...
    # ---- key robustness: only plot levels that exist ----
    # default_order = df_filtered[x].unique().tolist()
    # present_levels = df_filtered.loc[df_filtered.Area == area, x].dropna().unique()
    # order = [o for o in default_order if o in present_levels]
    if order is None:
        order = df_filtered[x].unique().tolist()
    fig, ax = plt.subplots(figsize=(2, 1.5))

    # Also guard hue levels: keep desired order but only those present
    wanted_hue = ['<10%', '>80%', 'all']
    present_hue = df_filtered.loc[df_filtered.Area == area, 'overlap_type'].dropna().unique()
    hue_order = [h for h in wanted_hue if h in present_hue]

    sns.violinplot(
        df_filtered[df_filtered.Area == area],
        y=y, x=x, hue='overlap_type',
        order=order, hue_order=hue_order, palette=palette[:len(hue_order)],
        linewidth=0, width=0.8, inner='box',
        inner_kws={'box_width': 1.25, 'color': '#191919'},
        cut=0, **args
    )

    # Legend
    legend = ax.legend(fontsize=6*0.7, loc=legend_loc, labelspacing=0.3)
    legend.set_title(title='overlap type', prop={'size': 6*0.8})
    legend.get_frame().set_linewidth(0.2)

    # # X ticks/labels — match number of plotted categories
    # label_map = {'image': 'image', 'SNR_spont': 'gray\nscreen', 'RS': 'lights\noff'}
    # xtick_labels = [label_map[o] for o in order]
    # ax.set_xticks(ticks=np.arange(len(order)), labels=xtick_labels)

    # Ax format
    ax.tick_params(axis='both', labelsize=6, width=0.5, pad=0.1)
    ax.set(xlabel=None)
    ax.set_ylabel('EV fraction', fontsize=6, labelpad=0)
    ax.spines[:].set_linewidth(0.5)
    sns.despine()

    # ---- stars: positions/heights adapt to how many x-levels you have ----
    if len(order) == 3:
        stars_positions = [
            [0.12, 0.17, 0.22],
            [0.45, 0.50, 0.55],
        ...
        stars_positions, heights_list = [], []

    # Call your star helper for each present x-level
    for i, x_level in enumerate(order):
        add_group_pairwise_stars_1_set_rf_overlap(
            df_filtered[df_filtered.Area == area],
            neuron_property=y,
            x=x,
            x1=x_level,                   # pass the category key, not the displayed label
            hue='overlap_type',
            ax=ax,
            fontsize=5,
            perm_t=True,
            stars1_positions=stars_positions[i],
            bar_halflength=0.1,
            height1=heights_list[i][0],
            height2=heights_list[i][1],
            height3=heights_list[i][2],
        )

    # Null line from control
    plot_null_line(df_filtered_control[df_filtered_control.Area == area], neuron_property='EV', ax=ax)
    return ax

"""Place stars above the appropriate hue positions in each x category."""
def p_to_stars(p):
	if not np.isfinite(p): return "n.s."
	return "***" if p < 1e-3 else ("**" if p < 1e-2 else ("*" if p < 0.05 else "n.s."))


def pairwise_within_x_using_your_tests(
    df,
    x="Dataset Type",
    hue="bin size (ms)",
    y="EV",
    *,
    x_order=None,
    hue_order=None,
    compare="vs_ref",          # 'vs_ref' (each hue vs first hue) or 'all_pairs'
    mc_method="holm",          # 'holm', 'fdr_bh', etc. (applied within each x)
    perm_t=True,
    perm_type="ind",           # 'ind' or 'paired'
    hierarchical=False,
	...
	"""
	For each x level, compute pairwise tests across hue levels using the user's
	stats functions, and correct p-values *within that x*.
	Returns: dict[x_level] = {'pairs': [(h1,h2),...], 'p_raw': np.array, 'p_adj': np.array}
	"""
	if x_order is None:
		x_order = list(df[x].dropna().unique())
	if hue_order is None:
		hue_order = list(df[hue].dropna().unique())

	results = {}
	for xi in x_order:
		sub = df[df[x] == xi].dropna(subset=[hue, y]).copy()

		# choose pairs
		if compare == "vs_ref":
			ref = hue_order[0]
			pairs = [(ref, h) for h in hue_order if h != ref]
		else:
			pairs = list(combinations(hue_order, 2))

		p_raw = []
		valid_pairs = []
		for h1, h2 in pairs:
			# print(f"Comparing {h1} vs {h2} within {x}={xi}... ", end="")
			pair_df = sub[sub[hue].isin([h1, h2])].copy()

			# build p-value using your helper that mirrors get_comparison_test_stars logic
			if hierarchical:
				p = hierarchical_permutation_test(
					data=pair_df.rename(columns={mouse_or_date: "Mouse Name"}),
					mouse_or_date="Mouse Name",
				...
				g1 = pair_df[pair_df[hue] == h1][y].to_numpy()
				g2 = pair_df[pair_df[hue] == h2][y].to_numpy()
				p = perm_test_paired(g1, g2)
			elif perm_t:
				g1 = pair_df[pair_df[hue] == h1][y].to_numpy()
				g2 = pair_df[pair_df[hue] == h2][y].to_numpy()
				p = perm_test(g1, g2, num_permutations=num_permutations, central_tendency=central_tendency)
			else:
				_, p = stats.ttest_ind(
					pair_df[pair_df[hue] == h1][y].to_numpy(),
					pair_df[pair_df[hue] == h2][y].to_numpy(),
		...
		if ok.sum() > 0:
			_, p_adj_ok, _, _ = multipletests(p_raw[ok], method=mc_method, alpha=0.05)
			p_adj[ok] = p_adj_ok

		results[xi] = {"pairs": valid_pairs, "p_raw": p_raw, "p_adj": p_adj}
	return results