...
from stats_functions import get_group_pairwise_stars
...
## control prediction performance by duration of activity recording and number of predictor neurons 
...
def get_control_evars_seed(input_resp, pred_resp, input_control_lengths, pred_control_lengths, seed=17, 
    ...
    """
    np.random.seed(seed)
    min_neuron_length = min(pred_control_lengths,input_control_lengths)
    # Randomly select neurons based on control lengths
    if pred_resp.shape[1]>min_neuron_length:
        pred_chosen_neurons = np.random.choice(np.arange(pred_resp.shape[1]), min_neuron_length, replace=False)
        pred_resp_used = pred_resp[:,pred_chosen_neurons]
    else:
        pred_resp_used = pred_resp
        pred_chosen_neurons = np.arange(pred_resp.shape[1])
    
    if input_resp.shape[1]> min_neuron_length:
        input_chosen_neurons = np.random.choice(np.arange(input_resp.shape[1]), min_neuron_length, replace=False)
        input_resp_used = input_resp[:,input_chosen_neurons]
    else:
        input_resp_used=input_resp
        input_chosen_neurons = np.arange(input_resp.shape[1])
    ...
    return evars, evars2, pred_chosen_neurons, input_chosen_neurons
...
def get_control_evars(animal_stats,input_control_lengths, pred_control_lengths, control_trial_length,  animal='mouse', area='L23',area2='L4',
                            dataset_types = ['SNR', 'RF_thin', 'RF_large'],num_seeds=2, w_size=25, shuffle_frames=False, seed=17, n_splits=10, 
                            sample_size=500, monkey='L', subsampled_indices=False):
    ...
    for condition_type in dataset_types:
        ...
        for mouse_or_date in animal_stats[condition_type]:
            if mouse_or_date in ['140819', '150819', '160819']:
                continue
            all_evars, all_evars2, all_pred_chosen_neurons, all_input_chosen_neurons = [],[],[],[]
            if animal=='mouse':
                ...
                pred_resp, input_resp= get_resps(condition_type=get_condition_type, date=mouse_or_date, w_size=w_size, 
                                                stim_on=0, stim_off=all_ini_stim_offs[condition_type], monkey=monkey)
                if subsampled_indices is True:
                    assert animal_stats[condition_type][mouse_or_date]['V1'][f'monkey_{monkey}_subsample_indices'] is not None, "subsampled_indices is set to True but no subsampled indices found in monkey_stats V1"
                    assert animal_stats[condition_type][mouse_or_date]['V4'][f'monkey_{monkey}_subsample_indices'] is not None, "subsampled_indices is set to True but no subsampled indices found in monkey_stats V4"
                    subsampled_indices_dict = {'V1':animal_stats[condition_type][mouse_or_date]['V1'][f'monkey_{monkey}_subsample_indices'],
                                                'V4':animal_stats[condition_type][mouse_or_date]['V4'][f'monkey_{monkey}_subsample_indices']}
                    pred_resp = pred_resp[:,subsampled_indices_dict[area]]
                    input_resp = input_resp[:,subsampled_indices_dict[area2]]
            ...
            if pred_resp.shape[0]> control_trial_length:
                ...
                for evar, evar2, pred_chosen_neurons, input_chosen_neurons in results:
                    all_evars.append(evar)
                    all_evars2.append(evar2)
                    all_pred_chosen_neurons.append(pred_chosen_neurons)
                    all_input_chosen_neurons.append(input_chosen_neurons)
                # evars = np.mean(np.array(all_evars), axis=0)
                # evars2=np.mean(np.array(all_evars2), axis=0) sep 2025 change 
                evars = np.array(all_evars)
                evars2=np.array(all_evars2)
                chosen_neurons = np.array(all_pred_chosen_neurons)
                chosen_neurons2 = np.array(all_input_chosen_neurons)
            else:
                ...
                #                                             input_alpha=input_alpha, pred_alpha=pred_alpha) was not subsampling over neuron lengths
                results = Parallel(n_jobs=-1)(delayed(get_control_evars_seed)(input_resp=input_resp, 
                                                                            pred_resp=pred_resp, 
                                                                            input_control_lengths = input_control_lengths,
                                                                            pred_control_lengths=pred_control_lengths,
                                                                            shuffle_frames=shuffle_frames, seed=seed,
                                                                            condition_type_=condition_type, n_splits=n_splits,
                                                                            input_alpha=input_alpha, pred_alpha=pred_alpha) for i in range(num_seeds)) 
                for evar, evar2, pred_chosen_neurons, input_chosen_neurons in results:
                    all_evars.append(evar)
                    all_evars2.append(evar2)
                    all_pred_chosen_neurons.append(pred_chosen_neurons)
            ...
            animal_stats[condition_type][mouse_or_date][area]['fig4_chosen_neurons']=chosen_neurons
            animal_stats[condition_type][mouse_or_date][area2]['fig4_chosen_neurons'] = chosen_neurons2
    ...
    for dataset_type in dataset_types:
        ...
        for mouse, areas_data in mouse_stats_[dataset_type].items():
            ...
            for area, values in areas_data.items():
                ...
                for neuron_index, (split_half_r, snr,max_corr_val, evar, null_evar) in enumerate(zip(split_half_rs, SNRs,values['max_corr_val'],values['stimulus_type_evars'],values['stimulus_type_evars_null'])):
                    data.append({
...
def make_monkey_df_stimulus_comparisons(monkey_stats_, dataset_types=['SNR', 'RF_thin', 'RF_large'], get_variances=False):
    # i made the change here sep 2025 to NOT average across seeds, but keep all evar values because 
    # these are neurons from the same animal. i can just get the mean over the individual neurons.
    data = []
    for dataset_type in dataset_types:
        if 'spont' in dataset_type:
            act_type = 'gray screen'
        ...
        for date, areas_data in monkey_stats_[dataset_type].items():
            ...
            for area, values in areas_data.items():
                if area=='V4':
                    ...
                    direction = 'V4→V1'
                num_seeds = len(values['fig4_chosen_neurons'])
                # print(f'num seeds for {dataset_type}, {date}, {area}: {num_seeds}')
                ...
                if get_variances is True:
                    if 'RF' in dataset_type:
                        ...
                        rf_variance_across_stimuli = [0]*len(split_half_rs)
                ...
                for s in range(num_seeds):
                    ...
                    # print all lengths to make sure they match
                    # print(f'lengths for seed {s}: split_half_rs: {len(split_half_rs)}, SNRs: {len(SNRs)}, max_corr_val: {len(values["max_corr_val"])}, evars: {len(values["stimulus_type_evars"][s])}, null_evars: {len(values["stimulus_type_evars_null"][s])}')
                    for n, (split_half_r, snr,max_corr_val, evar, null_evar) in enumerate(zip(split_half_rs, SNRs,
                                                                                        values['max_corr_val'],
                                                                                        values['stimulus_type_evars'][s],
                                                                                        values['stimulus_type_evars_null'][s]),
                                                                                        ):
                        data.append({
                            'Dataset Type': dataset_type,
                            'Activity Type': act_type,
...
def add_violin_custom_colors(ax):
    ...
    # custom_colors = ['#4E8EEC','#A3C4F7','#FF6256','#FFA79F'] # new colors with directionalities 
...
def add_stars_2_sets(df_, neuron_property, x, x_order, hue, ax, fontsize=7, height1=0.97, height2=0.97, perm_t=True, perm_type='ind', hierarchical=False, mouse_or_date='Mouse_Name', central_tendency='median'):
    stars1 = get_comparison_test_stars(df_[df_[x]==x_order[0]], hue, neuron_property, perm_t=perm_t, perm_type=perm_type, hierarchical=hierarchical, mouse_or_date=mouse_or_date, central_tendency=central_tendency)
    stars2 = get_comparison_test_stars(df_[df_[x]==x_order[1]], hue, neuron_property, perm_t=perm_t, perm_type=perm_type,hierarchical=hierarchical, mouse_or_date=mouse_or_date, central_tendency=central_tendency)
    if stars1 == 'n.s.':
...
def add_violin_custom_colors_3(ax, custom_colors=None):
...
def plot_neuronal_properties_comparison(df_, variable1, neuron_property, variable2, 
                                        ...
                                        mouse_height1=0.97,mouse_height2=0.97, return_pvals=False,central_tendency='median',
                                        **plot_args):
    ...
    if animal == 'mouse':
        ...
        add_stars_2_sets(df_, neuron_property, x=variable1, x_order=label_order,
                         hue=variable2, ax=ax, hierarchical=True, central_tendency=central_tendency, **stats_args)
    else:
        add_violin_custom_colors_3(ax)
        pval_dict = add_group_pairwise_stars_2_sets(df_, neuron_property, x=variable1, x_order=label_order, hue=variable2, 
                               hue_order=hue_order, ax=ax, return_p_values=True, **stats_args)
