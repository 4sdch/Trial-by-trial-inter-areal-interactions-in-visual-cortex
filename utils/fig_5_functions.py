            ...
            for area, values in areas_data.items():
                ...
                if spont_comparisons is True:
                    ...
                    if 'spont' not in dataset_type or 'RS' in dataset_type:
                        ...
                        if 'spont_comparison_evars' in values.keys():
                            evars = values['spont_comparison_evars']
                            evars_null = values['spont_comparison_evars_null']
                            if len(evars.shape)>1:
                                evars = np.mean(evars, axis=0)
                            ...
                            print('Using regular evars bc spont comparison evars not found for dataset type:', dataset_type, 'date:', date, 'area:', area)
                            evars = values['evars']
                ...
                print('split_half_rs shape:', np.array(split_half_rs).shape,'SNRs shape:', np.array(SNRs).shape,'evars shape:', np.array(evars).shape,'evars_null shape:', np.array(evars_null).shape)
                for n_idx, (split_half_r, snr,max_corr_val, evar, null_evar) in enumerate(zip(split_half_rs, SNRs,values['max_corr_val'],evars,evars_null)):
                    data.append({
                        'Dataset Type': dataset_type,
                        'Activity Type': act_type,
                        'Date':date,
...
def add_group_pairwise_stars_2_sets_5_conditions(df_, neuron_property, x, x_order, hue, hue_order, ax, fontsize=5, height1=0.73, 
    ...
    The significance stars indicate the level of significance for each pairwise comparison: *** for p < 0.001, ** for p < 0.01, * for p < 0.05, and 'n.s.' for not significant.
    """
    labels1,all_stars1 = get_group_pairwise_stars(df_[df_[x]==x_order[0]], hue, hue_order, neuron_property, perm_t=True, perm_type='paired')
    _, all_stars2 = get_group_pairwise_stars(df_[df_[x]==x_order[1]], hue, hue_order, neuron_property,perm_t=True, perm_type='paired')
    ...
    for s, (star1, star2) in enumerate(zip(all_stars1[:4], all_stars2[:4])):
        if s>0:
            ax.hlines(height_positions1[s]- 0.01, stars1_positions[s] - bar_halflength-offset_, stars1_positions[s] + bar_halflength+offset_, 
                        color='black',transform=ax.transAxes, linewidth=0.5, clip_on=False)
...
def add_group_pairwise_stars_2_sets_3_conditions(df_, neuron_property, x, x_order, hue, hue_order, ax, fontsize=5, height1=0.73, 
                        height2=0.82, stars1_positions=None, stars2_positions=None):
    """
    Add significance stars to a grouped bar plot representing two sets of data with five conditions.
    ...
    labels1,all_stars1 = get_group_pairwise_stars(df_[df_[x]==x_order[0]], hue, hue_order, neuron_property, perm_t=True, perm_type='paired')
    _, all_stars2 = get_group_pairwise_stars(df_[df_[x]==x_order[1]], hue, hue_order, neuron_property,perm_t=True, perm_type='paired')
    height_positions1 = [height1,height2]
    height_positions2 = [height1,height2]
    
    print('length of all_stars1:', len(all_stars1))
    print('length of all_stars2:', len(all_stars2))
    if stars1_positions is None:
        stars1_positions= [0.14,0.19]
    if stars2_positions is None:
        stars2_positions= [0.65, 0.7]
    bar_halflength = 0.06
    offset_=0
    for s, (star1, star2) in enumerate(zip(all_stars1[:2], all_stars2[:2])):
        if s>0:
            ax.hlines(height_positions1[s]- 0.01, stars1_positions[s] - bar_halflength-offset_, stars1_positions[s] + bar_halflength+offset_, 
...
def fig5_violinplot(df, x, y, hue, ax, y_label, linewidth=0.9,
                    ...
                    animal='mouse',show_legend=False,leg_loc=(1,0.3), monkey_name = 'L',
                    height1=0.73, height2= 0.82, height3 = 0.91,height4=1, stars1_positions=None, stars2_positions=None,**args):
    label_order=['V1→V4', 'V4→V1']
    if animal =='monkey' and monkey_name != 'A':
    ...
    sns.despine()
    ax.set_ylabel(y_label, fontsize=fontsize, labelpad=0)
    ax.set_xlabel(None)
    ax.legend_.remove()
    ...
    if animal =='monkey' and monkey_name != 'A':
        add_group_pairwise_stars_2_sets_5_conditions(df[df['control_shuffle']==False], y, x, label_order, hue, 
                                            hue_order, ax, fontsize=5, height1=height1, height2=height2, 
                                            height3=height3,height4=height4, stars1_positions=stars1_positions,
                                            stars2_positions=stars2_positions)
        ...
        add_stars_2_sets(df[df['control_shuffle']==False], y, x, label_order, hue, ax, 
                    hierarchical=False, height1=0.7,height2=0.7, perm_type='paired')
        
    ax.tick_params(axis='both', labelsize=fontsize, width=0.5, length=3, pad=2)
    add_custom_colors(ax, animal=animal, monkey_name=monkey_name)
    

def add_custom_colors(ax, animal='mouse', monkey_name='L'):
...
def fig5_violinplot_3_conditions(df, x, y, hue, ax, y_label, linewidth=0.9,
                    palette=['gray','lightgray'], fontsize=7,plot_control_ev=True, 
                    animal='mouse',show_legend=False,leg_loc=(1,0.3), monkey_name = 'L',
                    height1=0.73, height2= 0.82, stars1_positions=None, stars2_positions=None, **args):
    label_order=['V1→V4', 'V4→V1']
    if animal =='monkey' and monkey_name != 'A':
        hue_order=['stimulus', 'gray screen','lights off']
    else:
    ...
    if animal =='monkey' and monkey_name != 'A':
        add_group_pairwise_stars_2_sets_3_conditions(df[df['control_shuffle']==False], y, x, label_order, hue, 
                                            hue_order, ax, fontsize=5, height1=height1, height2=height2, stars1_positions=stars1_positions, stars2_positions=stars2_positions)
    elif animal == 'mouse':
        add_stars_2_sets(df[df['control_shuffle']==False], y, x, label_order, hue, ax, 
                    hierarchical=True, height1=0.7,height2=0.7, perm_type='paired')
    else:
    ...
    ax.tick_params(axis='both', labelsize=fontsize, width=0.5, length=3, pad=2)
    add_custom_colors_3_conds(ax, animal=animal, monkey_name=monkey_name)
    
def add_custom_colors_3_conds(ax, animal='mouse', monkey_name='L'):

    if animal =='monkey' and monkey_name != 'A':
        custom_colors = ['#72BEB7','#B6E3DF','#136a66',
...
def fig5_supp_violinplot(df,area, x, y, hue, ax, y_label, linewidth=0.9,
    ...
    **args : dict, optional
    ...
    for m, mouse in enumerate(label_order):
        ...
        star = get_comparison_test_stars(df[(df['control_shuffle']==False)&(df.Area==area)&(df[x]==mouse)], hue, y, perm_t=True, perm_type='paired', hierarchical=False)
        if star == 'n.s.':
