...
class mt_retriever:
    ...
    stim_spont_filemanames = ['stimspont_M170604_MP031_20170627.mat','stimspont_M170714_MP032_20170801.mat','stimspont_M170717_MP033_20170825.mat']
    # omitting natimg32_M150824_MP019_2016-03-23 and 'stimspont_M150824_MP019_20160323.mat' due to no cells in indices 385 and 420
    def __init__(self, main_dir, dataset_type):
        ...
        elif 'stimspont' in dataset_type:
            self.filenames = copy.deepcopy(self.stim_spont_filemanames)
        for dataset in self.filenames:
            with open(os.path.join(main_dir,f'data/stringer/{dataset}'), mode='rb') as file: # b is important -> binary
    ...
    def set_raw_responses(self):
        ...
        if 'spont' in self.mt['stim'][0].dtype.names:
            spont = self.mt['stim'][0]['spont'][0]  # timepoints by neurons
        else: # this is the funky stimspont dataset where spont is not given. i need the raw responses then will bin like authors did
            stimpt = self.mt['stimtpt'][:,0]
            stimpt = stimpt.astype(np.float64)
            spontaneous_activity_unbinned_indices = np.where(stimpt == 0)[0]
            onsets = spontaneous_activity_unbinned_indices[::3]
            spont = np.zeros((len(onsets), len(self.mt['Fsp'])))
            for i, onset in enumerate(onsets):
                segment = self.mt['Fsp'][:,onset : onset + 3]
                spont[i] = segment.mean(axis=1)
    ...
    def retrieve_layer_activity(self, activity_type, dataset_name, mean_center=False, removed_pc = False):
        ...
        # print(self.mt['stim']['resp'][0][0].shape)
